<?php
	...
	foreach($verif as $element){
		if($element['credit']-100 < 0){
			$text = 'Vous n\'avez pas assez de credit !!';
			$text2 = 'Veillez attendre d\'en recevoir plus.';
		}
		else{
			$donnees = $base->prepare('UPDATE Users SET Credit = Credit-100 WHERE IdUsers = :id');
    		$donnees->execute(array(':id'=>$_SESSION['id']));
			$text = 'Vous avez vendu 100 crédit !!';
			$text2 = 'Profité bien de vos 10€.';
		}
	}
?>
